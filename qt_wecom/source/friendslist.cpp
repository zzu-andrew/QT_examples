...
void CFriendsList::InitList()
{
    ...
    for (int i = 1; i <= 108; i++)
    {
        ...
        int nNameIndex = QRandomGenerator::global()->bounded(strListName.length());

        pLabelName->setText(strListName.at(nNameIndex));
        pLabelName->setStyleSheet("font: 14px; color:#000000;");

        int nTimeIndex = QRandomGenerator::global()->bounded(strListTime.length());
        ...
        int nMsgIndex = QRandomGenerator::global()->bounded(strListMsg.length());
